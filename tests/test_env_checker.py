import pytest
from fastapi_kickstart.env_checker import check_environment

def test_check_environment(mocker):
    # Mock the functions that check for dependencies
    mock_check_pip = mocker.patch('fastapi_kickstart.env_checker.check_pip')
    mock_check_poetry = mocker.patch('fastapi_kickstart.env_checker.check_poetry')
