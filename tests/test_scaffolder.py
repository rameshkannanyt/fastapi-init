import pytest
from fastapi_kickstart.scaffolder import FastAPIKickstart

@pytest.fixture
def fastapi_kickstart():
    return FastAPIKickstart()

def test_create_project_scaffold(fastapi_kickstart, tmp_path):
    project_name = "test_project"
    project_path = tmp_path / project_name

    # Create project scaffold
    result_path = fastapi_kickstart.create_project_scaffold(project_name, base_path=tmp_path)
...
def test_create_project_scaffold_existing_directory(fastapi_kickstart, tmp_path):
    ...
    with pytest.raises(ValueError, match=f"Project directory {project_path} already exists"):
        fastapi_kickstart.create_project_scaffold(project_name, base_path=tmp_path)

def test_generate_files(fastapi_kickstart, tmp_path):
    project_name = "test_project"
    project_path = tmp_path / project_name
    fastapi_kickstart.create_project_scaffold(project_name, base_path=tmp_path)
